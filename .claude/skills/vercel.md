...
## Monorepo: Next.js + Python FastAPI (Hybrid Deployment)

### Overview

Deploying a monorepo with Next.js frontend AND Python FastAPI backend requires a specific pattern. This is different from the standard monorepo pattern above.

<EXTREMELY-IMPORTANT>
Do NOT use `rootDirectory` for hybrid Next.js + Python deployments. Setting rootDirectory blocks Vercel from detecting the `api/` folder for Python functions.
</EXTREMELY-IMPORTANT>
...
```
repo-root/
├── api/                    # Python serverless functions (at repo root!)
│   └── index.py           # FastAPI ASGI handler
├── backend/               # Python modules (NOT in api/)
│   ├── __init__.py
│   ├── api/
│   ├── models/
│   └── services/
...
│   ├── package.json
│   └── next.config.js
├── package.json           # Root package.json (REQUIRED for framework detection)
└── vercel.json
```
...
<EXTREMELY-IMPORTANT>
Vercel requires a `next` dependency in the root package.json for framework detection, even when Next.js is in a subdirectory.
</EXTREMELY-IMPORTANT>
...
Without this, you'll get: `Error: No Next.js version detected`
...
**Key points:**
- `installCommand` and `buildCommand` cd into frontend directory
- `outputDirectory` points to frontend/.next
- `framework: "nextjs"` must be set for proper Next.js serving
- `functions.includeFiles` bundles Python modules from outside `api/`
- Rewrites route all `/api/*` requests to the Python handler

### Python FastAPI Handler (api/index.py)
...
repo_root = Path(__file__).parent.parent  # api/ -> repo root
if str(repo_root) not in sys.path:
    sys.path.insert(0, str(repo_root))

from backend.api import domains, credentials, auth
...
**Error: `The pattern '../api/**/*.py' doesn't match any Serverless Functions`**

- **Cause:** `rootDirectory` is set, making api/ inaccessible
- **Fix:** Remove rootDirectory (see "Clearing rootDirectory" below)

**Error: `No Next.js version detected`**

- **Cause:** No `next` dependency in root package.json
- **Fix:** Create root package.json with `next` in dependencies

**Error: Frontend returns 404 / `framework: null`**

- **Cause:** Vercel doesn't know how to serve Next.js output
- **Fix:** Set `"framework": "nextjs"` in vercel.json AND have root package.json

**Error: `pnpm ERR_INVALID_THIS`**

- **Cause:** pnpm version compatibility issues on Vercel
- **Fix:** Use npm instead of pnpm in install/build commands

### Clearing rootDirectory via Vercel API

If you previously set rootDirectory (e.g., to "frontend"), you MUST clear it:

```bash
# Get auth token location
cat ~/Library/Application\ Support/com.vercel.cli/auth.json

# Get project and team IDs
vercel project inspect your-project

# Clear rootDirectory via API
...
**Important:** The Vercel dashboard doesn't allow clearing rootDirectory to null—you must use the API.

### Neon PostgreSQL Configuration

For Neon databases with connection pooling, set both pooled and non-pooled URLs:

```bash
# Production (pooled for serverless)
printf 'postgresql://user:pass@ep-xxx-pooler.region.aws.neon.tech/db?sslmode=require' | \
  vercel env add DATABASE_URL production

# Production (non-pooled for migrations)
printf 'postgresql://user:pass@ep-xxx.region.aws.neon.tech/db?sslmode=require' | \
...
**When to use which:**
- **Pooled (with `-pooler` in hostname):** For serverless functions (handles connection limits)
- **Non-pooled:** For migrations, long-running scripts, or when you need persistent connections

### Testing Locally

Use `vercel dev` to test the full hybrid setup:

```bash
vercel dev
```

This properly routes:
- `/api/*` → Python FastAPI functions
- All other routes → Next.js frontend

### Deployment Checklist

1. ☐ Root package.json exists with `next` dependency
2. ☐ No `rootDirectory` set (or cleared via API)
3. ☐ `api/` folder at repo root (not in frontend/)
4. ☐ `functions.includeFiles` includes Python modules
5. ☐ Using npm (not pnpm) in build commands
6. ☐ `framework: "nextjs"` in vercel.json
7. ☐ Database URLs set for all environments
